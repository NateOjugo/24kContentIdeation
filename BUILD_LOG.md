...
## Template ordering + citation integrity fix (2026-08-08, later)

**Scaffold-first ordering** (`hook_template_library`): `has_placeholder=true` rows now rank above finished one-liners within each format, and non-placeholder rows are admitted only when a format has fewer than `MIN_PLACEHOLDER_TEMPLATES` (3) scaffolds. Rationale: scaffolds rewrite into the 24K voice, finished swipe-file lines mostly do not. Per the cross-tab this fallback fires for exactly two formats — Contrarian (1 scaffold) and Case Study (1) — while the other six have 4-66 and stay scaffolds-only.

**Fixed a latent truncation bug** introduced with the library: `fetchHookTemplateLibrary` had `limit = 40` while Personal Experience holds 83 rows. Ordered by `niche_relevant desc`, all 40 fetched rows came back niche-relevant, so `genericRows` was always empty and **broadening silently did nothing for that format**. Limit raised to 120 (above the largest per-format set) with the reason documented at the call site.

**TEMPLATE USED was fabricable — now checkable.** A verification run cited `Library template "i've always been [trait or preference], but [something unexpected]."` That template does not exist in the table and is not a paraphrase of any of the 66 available Personal Experience scaffolds; the model invented it. Since the field's whole purpose is provenance, an uncheckable claim is worse than none. Candidates are now enumerated with stable labels (`P1..Pn` proven, `L1..Ln` library), the prompt requires citing one verbatim, and `verifyTemplateCitation()` rewrites the line server-side when it cites a label that was never offered. Re-running the same generation produced an honest `none — written from scratch. None of the L1-L12 scaffolds mapped cleanly...`.

**Finding worth acting on:** across two honest runs — including Personal Experience, the library's best case with 66 niche scaffolds — the model consulted the library and declined it both times. The plumbing works and precedence is enforced, but the swipe-file material is generic creator-economy phrasing that fights Cold Iron voice. Real lift will likely come from seeding `hook_formats` with proven templates pulled from actual posted scripts, not from growing the library.

## Anthropic API key rotated (2026-08-09)

New key created in the Anthropic console, named `24kscriptingv1`. Set in Vercel production env (`nate-ojugos-projects/24k-script-vault`), local `.env.local` updated, production redeployed, and confirmed live with a real generation against `masculinity-identity` (draft saved, verified, then deleted as a test row — not kept in the Vault).

**Verification gap, noted rather than papered over:** I never captured the old key's raw value at any point this session (only ever printed its fingerprint, `...zQAA`), so I have no way to independently confirm it was revoked in the console — that step relies on Nate's own action there. Everything downstream (Vercel, local, live generation) is directly verified.
